...
export default function ReportsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof ApiClientError ? err.message : 'Не удалось загрузить отчёты');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <main>
      <PageContainer>
        <PageHeader
          title="Отчёты"
          description="KPI продаж, заказов, склада и юнит-экономика на базе существующих analytics endpoints"
          breadcrumbs={[
        ...
        {loading ? <LoadingState message="Загрузка отчётов…" /> : null}
              ...
              <Card title="Складские операции">
                <div className="metric-grid">
                  ...
                  <MetricCard
                    ...
                    value={inventoryCountProgress.filter(
                      (item) => item.status !== 'POSTED' && item.status !== 'CANCELLED',
                    ).length}
